...
function Home() {
  ...
  const getDataFromApi = async () => {
    try {
      ...
      const nasional = await axios.get(
        `${process.env.REACT_APP_CNN_API}/nasional`
      );
      const internasional = await axios.get(
        `${process.env.REACT_APP_CNN_API}/internasional`
      );
      const ekonomi = await axios.get(
        `${process.env.REACT_APP_CNN_API}/ekonomi`
      );
      const olahraga = await axios.get(
        `${process.env.REACT_APP_CNN_API}/olahraga`
      );
      const teknologi = await axios.get(
        `${process.env.REACT_APP_CNN_API}/teknologi`
      );
      const hiburan = await axios.get(
        `${process.env.REACT_APP_CNN_API}/hiburan`
      );
      const gayaHidup = await axios.get(
        `${process.env.REACT_APP_CNN_API}/gaya-hidup`
      );
  ...
  return (
    <div>
      <Helmet>
        <meta charSet="utf-8" />
          <title>CNN Indonesia | Berita Terbaru, Terikini Indonesia, Dunia</title>
      </Helmet>
          ...
          <div className="main-left col-lg-8 p-3">
            ...
            <div className="gap-component-content">
              <LaporanInteraktif
                ...
                background={newsUpdateBackground}
                titleColor={newsUpdateTitleColor}
                subTitleColor={newsUpdateSubTitleColor}
              />
            </div>
            <div className="gap-component-content">
              <ListNewsUpdate />
